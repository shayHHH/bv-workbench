...
- 已修：1.1.1 / 1.1.3（实收校验+真实差额异常）、1.1.4（confirms 必填不再默认 20；链/方式选项仍写死，待渠道配置）、1.2.3 / 1.2.4（切类型清报价+提示；选报价联动买入额，cost/fee 因 quote_records 无字段遗留）、1.2.6（汇率不一致二次确认；金额×汇率容差校验未做）、1.2.10（改存 scenario_id，徽标按 id 联查）、1.3.1（本地日期单号）、1.4.1 / 1.4.2（person_name 建单字段；出款账户可编辑落库）、1.4.5（比例集中 PROFIT_RATE_CONFIG，界面"估算"标注在 OrderPanel 侧）、1.4.7 / 1.4.8（缺账户/余额不足 400 阻断+警戒线预警）、1.4.10（通道余额按订单币种）、1.4.12（冻结随渠道迁移）、1.4.13（handler_name 随状态推进更新）、3.5 部分（TO-/SCH- 前缀跳订单页）
- 3.4 环境标示已改（AppLayout 混有并行会话未提交改动，随其一并提交）
- 已修（审计会话，commit 0065ce9 + 后续）：1.1.2（实收金额/方式/登记人上屏，差额括注）、1.2.7（详情「关联报价」区块，含与执行汇率不一致告警）、1.2.9 展示侧（业务类型空值显示「未指定（按客户整体准入）」）、1.4.5 展示侧（收益卡「估算」标注）、2.1/2.2/2.4/2.5（seed 单号从 created_at 本地时区派生、排单时间序、treasury 对账自洽、TO-102 报价快照指向真实 quote_records、人名对齐真实账号 sinclair/jacky/choy）
- 未修遗留：1.1.5（渠道配置化）、1.2.1/1.2.2（预设汇率表接基准价）、1.2.5 cost/fee 字段（quote_records 需扩展）、1.2.11（EXPIRED 过期扫描）、1.4.9（opening/floor 读取）、1.4.14（receipt_ref 渲染）、2.3（汇率方向标注）、3.1/3.3（交易员下拉接 users）、3.5 全局搜索分发（TO-/SCH- 前缀已做）、3.6 工作台（等用户选方案）、3.7（客户详情订单页签）、3.8（中介预期加点口径）
